...
/**
 * 🔥 REGISTRATION SYNC - Real-time on registration changes
 */
export const syncRegistrationToKV = onDocumentWritten(
  ...
  async (event) => {
    try {
      ...
      if (!SYNC_SECRET) {
        console.error('❌ SYNC_SECRET missing. Skipping registration sync.');
        return { success: false, error: 'SYNC_SECRET missing' };
      }
      ...
      console.log(`✅ Synced registration: ${event.params.registrationId}`);
      return { success: true, status: response.status };

    } catch (err) {
      console.error('❌ Registration sync failed:', err instanceof Error ? err.message : err);
      return { success: false, error: err instanceof Error ? err.message : 'Unknown error' };
    }
  }
);
