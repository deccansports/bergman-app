...
import { getClubRankingData } from '@/lib/actions/clubActions'; // Ensured import
import type { ClubRankingEntry, ClubAthleteContribution } from '@/lib/types';
import { Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter } from '@/components/ui/card';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Loader2, LogIn, Search, CalendarDays, FilterX, Building, Instagram, Facebook, ChevronDown, Users, Star } from 'lucide-react';
...
function ClubRankingsDisplay() {
  ...
  const availableYears = useMemo(() => {
    ...
    const maxYear = currentActualYear; // Cap at current year
    ...
    <main className="container mx-auto py-8 px-4 flex-grow">
        <Card className="shadow-xl rounded-xl overflow-hidden border-t-4 border-primary">
          <CardHeader className="bg-primary/5 text-center">
            ...
            <CardTitle className="text-3xl font-bold text-primary">Club Rankings - India</CardTitle>
                        ...
                        <React.Fragment key={club.clubId}>
                          <TableRow className={`even:bg-muted/20 hover:bg-primary/5 transition-colors ${openAccordionItem === club.clubId ? 'bg-primary/10' : ''}`}>
                            ...
                            <TableCell className="p-2 text-center">
                              {club.logoUrl ? (
                                <Image src={club.logoUrl} alt={`${club.clubName} logo`} width={40} height={40} className="rounded-full object-cover inline-block border" data-ai-hint="club logo" loading="lazy" />
                              ) : (
