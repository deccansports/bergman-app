// src/components/bikeRack/BikeRackAdminTab.tsx
"use client";
...
import { Loader2, Bike, PlusCircle, Trash2, Save, Send, UserCheck, Download, Search } from 'lucide-react';
...
interface BikeRackAdminTabProps {
  events: EventCalendarEntry[];
  isLoadingEvents: boolean;
}

export default function BikeRackAdminTab({ events, isLoadingEvents }: BikeRackAdminTabProps) {
  const [selectedEventId, setSelectedEventId] = useState<string>('');
  const { assignments, setAssignments, stats, isLoading, saveAssignments, sendNotifications, refresh } = useBikeRack(selectedEventId);
  ...
  const handleAssignmentChange = (index: number, field: 'rackName' | 'bibFrom' | 'bibTo', value: string | number) => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    const result = await saveAssignments(assignments);
    if (result.success) {
        toast({ title: 'Success', description: 'Assignments saved.' });
        refresh();
    } else {
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        <Select onValueChange={setSelectedEventId} disabled={isLoadingEvents} value={selectedEventId}>
          ...
          <SelectContent>{(upcomingEvents || []).map(e => <SelectItem key={e.id} value={e.id}>{e.eventName}</SelectItem>)}</SelectContent>
        </Select>
        ...
        {selectedEventId && (
            <>
                ...
                <form onSubmit={handleSearchBib} className="flex gap-2">
                  ...
                  />
                  <Button type="submit" disabled={isBibSearching || !searchTerm}>
