// src/components/volunteer/BikeLoopTab.tsx
"use client";
...
import { Loader2, Search, Bike, PlusCircle, MinusCircle } from 'lucide-react';
import { searchParticipantsForCheckInAction, recordLoopAction } from '@/lib/actions';

interface BikeLoopTabProps {
  eventId: string;
  volunteerId: string;
...
export default function BikeLoopTab({ eventId, volunteerId, volunteerName, eventDetails }: BikeLoopTabProps) {
  ...
  const handleRecordLoop = useCallback(async (increment: boolean) => {
    ...
    try {
      ...
      if (result.success && result.participant) {
        toast({ title: 'Success', description: `Bike loop count for BIB ${participant.bibNumber} updated.` });
  ...
  const { requiredLoops, completedLoops } = useMemo(() => {
    ...
    return { 
        requiredLoops: ticket?.bikeLoops || 0, 
        completedLoops: participant.bikeLoopsCompleted || 0 
    };
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Bike className="h-5 w-5 text-primary"/>Bike Loop Counter</CardTitle>
        <CardDescription>Enter a BIB number to record a completed bike loop.</CardDescription>
      </CardHeader>
      <CardContent>
