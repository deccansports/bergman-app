// src/components/volunteer/SwimLoopTab.tsx
"use client";
...
import { Loader2, Search, Waves, PlusCircle, MinusCircle } from 'lucide-react';
import { searchParticipantsForCheckInAction, recordLoopAction } from '@/lib/actions';

interface SwimLoopTabProps {
  eventId: string;
  volunteerId: string;
...
export default function SwimLoopTab({ eventId, volunteerId, volunteerName, eventDetails }: SwimLoopTabProps) {
  ...
  const handleSearch = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Error', description: `Search failed: ${e.message}` });
    } finally {
  ...
  const handleRecordLoop = useCallback(async (increment: boolean) => {
    ...
    try {
      ...
      if (result.success && result.participant) {
        toast({ title: 'Success', description: `Swim loop count for BIB ${participant.bibNumber} updated.` });
      ...
      toast({ variant: 'destructive', title: 'Error', description: `Failed to record loop: ${e.message}` });
    } finally {
  ...
  const { requiredLoops, completedLoops } = useMemo(() => {
    ...
    return { 
        requiredLoops: ticket?.swimLoops || 0, 
        completedLoops: participant.swimLoopsCompleted || 0 
    };
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Waves className="h-5 w-5 text-primary"/>Swim Loop Counter</CardTitle>
        <CardDescription>Enter a BIB number to record a completed swim loop.</CardDescription>
      </CardHeader>
      <CardContent>
