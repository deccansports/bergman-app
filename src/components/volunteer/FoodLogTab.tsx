// src/components/volunteer/FoodLogTab.tsx
"use client";
...
export default function FoodLogTab({ eventId }: FoodLogTabProps) {
  ...
  const fetchFoodStats = useCallback(async (currentEventId: string) => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: `Could not fetch food stats: ${errMsg}` });
    }
  ...
  const handleIssueMeal = async () => {
    if (!eventId || !searchedParticipant?.id || !mealToIssue) return;
    setIsIssuing(mealToIssue);
    try {
      const result = await markItemIssuedAction(eventId, searchedParticipant.id, mealToIssue);
      if (result.success) {
        toast({ title: 'Success', description: result.message });
        await handleSearch(); // Refresh participant data
        await fetchFoodStats(eventId); // Refresh stats
      } else {
  ...
  const handleResetMeal = async (mealType: 'Breakfast' | 'Lunch') => {
      if (!eventId || !searchedParticipant?.id) return;
      setIsResetting(mealType);
      try {
        const result = await resetIssuedItemStatusAction(eventId, searchedParticipant.id, mealType);
        if (result.success) {
            toast({ title: 'Success', description: result.message });
            await handleSearch(); // Refresh data
            await fetchFoodStats(eventId);
        } else {
            toast({ variant: 'destructive', title: "Reset Failed", description: result.message });
        }
      } catch (error: unknown) {
        const errMsg = error instanceof Error ? error.message : 'Unknown error';
        toast({ variant: 'destructive', title: "Error", description: `Could not reset status: ${errMsg}` });
      } finally {
        setIsResetting(null);
      }
  };
  ...
  return (
    <Card className="bg-background shadow-md">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-primary"><UtensilsCrossed className="h-5 w-5"/>Food</CardTitle>
      ...
      <CardContent className="space-y-4">
        {isLoadingStats && eventId ? <p>Loading stats...</p> : eventId && foodStats && (
