...
import { serializeValue, serializeParticipantDataUtil, normalizeToE164, toIsoStringSafe } from '@/lib/utils';
...
export async function createAndAssignVolunteerAction(data: CreateVolunteerUserActionInput): Promise<{ success: boolean; message: string; userId?: string }> {
    ...
    try {
        ...
        if (!normalizedMobile) {
          return { success: false, message: "Mobile number must be a valid international format (e.g., +919876543210)." };
        }

        const adminAuth = getAuthInstance();
        const adminDb = getFirestoreInstance();
        const newUserRecord = await adminAuth.createUser({ email, password, displayName: name, phoneNumber: normalizedMobile });
