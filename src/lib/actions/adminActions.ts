  ...
  searchBy: 'name' | 'email' | 'mobile' | 'bibNumber'
): Promise<{ success: boolean; message: string; athletes?: Array<User & { races?: RaceResult[]; upcomingEvents?: { eventId: string; eventName: string }[] }> }> {
    ...
    try {
        ...
        const athletesWithRacesPromises = userDocs.map(async (userDoc) => {
            ...
            const upcomingEvents: { eventId: string; eventName: string }[] = [];
