...
import { Satellite, Calendar, History, Loader2, ArrowRight, ImageIcon } from 'lucide-react';
import { getCalendarEventsAction } from '@/lib/actions/eventActions';
import type { EventCalendarEntry } from '@/lib/types';
import { parseISO, isToday, isPast, startOfDay, isAfter } from 'date-fns';
  ...
  const renderEventList = (eventList: EventCalendarEntry[], title: string, icon: React.ElementType) => (
    <div className="space-y-6">
      ...
      {eventList.length > 0 ? (
            ...
            <Card key={event.id} className="group relative flex w-full flex-col overflow-hidden rounded-lg border border-border shadow-lg transition-shadow duration-300 hover:shadow-xl">
              ...
              <div className="flex flex-1 flex-col p-4">
                <CardHeader className="mb-2 p-0">
                  ...
                  <CardDescription className="text-sm">{buildEventCategoryDateSummary(event)}</CardDescription>
                </CardHeader>
        ...
        <p className="italic text-muted-foreground">No {title.toLowerCase()} at the moment.</p>
      )}
    </div>
  );
...
export default function LiveTrackingIndexPage() {
  return (
    <>
      <section className="w-full bg-gradient-to-r from-primary/10 via-background to-background py-12 md:py-16 lg:py-20">
        <div className="container mx-auto px-4 text-center md:px-6">
          <h1 className="text-4xl font-bold tracking-tighter text-primary sm:text-5xl md:text-6xl">
            Bergman Live Tracking
          </h1>
          <p className="mx-auto mt-4 max-w-[700px] text-muted-foreground md:text-xl">
            Select an event to view the live race map, athlete leaderboard, and real-time progress.
          </p>
        </div>
      </section>
      <section className="w-full py-12 md:py-16">
        <div className="container mx-auto px-4 md:px-6">
          <Suspense fallback={<div className="flex items-center justify-center py-20"><Loader2 className="h-12 w-12 animate-spin text-primary" /></div>}>
            <LiveTrackingContent />
          </Suspense>
        </div>
      </section>
