...
import { getPaidFoodItemsAction, createPaidFoodOrderAction, verifyPaidFoodPaymentAction } from '@/lib/actions/paidFoodActions';
import type { EventCalendarEntry, PaidFoodItem, PaidFoodCoupon, PaidFoodOrder } from '@/lib/types';
import { Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from '@/components/ui/form';
import { Loader2, UtensilsCrossed, Minus, Plus, ShoppingCart, ArrowLeft, CheckCircle, Info, AlertTriangle } from 'lucide-react';
import { internationalMobileRegex } from '@/lib/utils';
import { Separator } from '@/components/ui/separator';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import { isPast, startOfDay, parseISO, isBefore } from 'date-fns';
...
export default function FoodPurchasePage() {
  ...
  const onSubmit = async (data: FoodPurchaseFormInput) => {
      ...
      try {
        const orderInput = {
            ...
            eventName: eventDetails?.eventName || 'General Purchase',
        ...
        const rzp = new window.Razorpay({
            ...
            handler: async (response: any) => {
                setIsSubmitting(true);
                const verificationResult = await verifyPaidFoodPaymentAction({
                    ...response,
                    ...orderResult.notes,
                });
                if(verificationResult.success) {
                    setOrderSuccessData({ orderId: orderResult.notes!.orderId });
                } else {
                    toast({ variant: "destructive", title: "Verification Failed", description: verificationResult.message, duration: 8000 });
                }
                setIsSubmitting(false);
            },
        ...
        rzp.on('payment.failed', (response: any) => { toast({ variant: "destructive", title: "Payment Failed", description: response.error.description }); setIsSubmitting(false); });
        rzp.open();
  ...
  const isEventOver = useMemo(() => {
    ...
    try {
        ...
        const eventDate = parseISO(eventDetails.eventDate);
  ...
  if (isEventOver) {
    return (
      <main className="flex-grow container mx-auto py-8 px-4 flex items-center justify-center min-h-[calc(100vh-10rem)]">
        <Card className="w-full max-w-lg text-center shadow-lg border-destructive/20">
          <CardHeader>
            <AlertTriangle className="h-12 w-12 text-destructive mx-auto" />
            <CardTitle className="text-destructive">Event Over</CardTitle>
            <CardDescription>
              Food purchases for <strong>{eventDetails?.eventName}</strong> are now closed as the event has already taken place.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground">We hope you had a great time!</p>
          </CardContent>
          <CardFooter>
                      ...
                      <div className="space-y-4">
                          ...
                          <div className="space-y-3">
                              {fields.map((field, index) => {
                                  ...
                                  return (
                                      <div key={field.key} className="p-3 border rounded-md">
                                        ...
                                        {itemDetails.description && (
                                            <p className="text-xs text-muted-foreground mt-2">{itemDetails.description}</p>
                                        )}
                                      </div>
                                  )
                              })}
