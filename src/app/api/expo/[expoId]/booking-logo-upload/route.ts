...
export async function POST(req: NextRequest, { params }: { params: { expoId: string } }) {
  try {
    ...
    if (!ALLOWED_MIME.has(mimeType)) {
      return NextResponse.json({ success: false, message: 'Only PNG, JPG and WEBP images are supported' }, { status: 400 });
    }

    const maxSizeBytes = 5 * 1024 * 1024;
    if (Number(file.size || 0) > maxSizeBytes) {
      return NextResponse.json({ success: false, message: 'Logo must be less than 5MB' }, { status: 400 });
    }
    ...
    const baseName = safeName(file.name.replace(/\.[^.]+$/, '') || 'logo');
    const objectName = `${Date.now()}-${baseName}.${ext}`;
    const objectPath = `expoBookingLogos/${eventId}/${expoId}/${objectName}`;
