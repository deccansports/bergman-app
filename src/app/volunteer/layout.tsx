...
export default async function VolunteerLayout({ children }: { children: ReactNode }): Promise<JSX.Element> {
  const sysRes = await getSystemControlAction();
  const isMaintenance = sysRes.success && sysRes.settings?.volunteerDashboardMaintenance;
  ...
  return (
    <Suspense fallback={<AthleteHubLoader />}>
      <ClientAuthGuard 
        ...
        redirectPath="/login?redirect=/volunteer/dashboard"
